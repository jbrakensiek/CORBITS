...
void print_results()
{
  ...
  if (analysis == 0) {
    fout_dist = fopen(nr?"all_mhs_dist_nr.txt":"all_mhs_dist.txt", "w");
    fout_hist = fopen(nr?"all_mhs_hist_nr.txt":"all_mhs_hist.txt", "w");
    fout_cdf  = fopen(nr?"all_mhs_CDF_nr.txt":"all_mhs_CDF.txt", "w");
    fout_py   = fopen("hist/all_mhs_hist_py.txt", "w");
    fout_R    = fopen("stat/all_mhs_hist_r.txt", "w");
  }
  else if (analysis == 1) {
    fout_dist = fopen(nr?"snr_mhs_dist_nr.txt":"snr_mhs_dist.txt", "w");
    fout_hist = fopen(nr?"snr_mhs_hist_nr.txt":"snr_mhs_hist.txt", "w");
    fout_cdf  = fopen(nr?"snr_mhs_CDF_nr.txt":"snr_CDF_mhs.txt", "w");
    fout_py   = fopen("hist/snr_mhs_hist_py.txt", "w");
    fout_R    = fopen("stat/snr_mhs_hist_r.txt", "w");
  }
  else {
    fout_dist = fopen(nr?"adj_mhs_dist_nr.txt":"adj_mhs_dist.txt", "w");
    fout_hist = fopen(nr?"adj_mhs_hist_nr.txt":"adj_mhs_hist.txt", "w");
    fout_cdf  = fopen(nr?"adj_CDF_mhs_nr.txt":"adj_CDF_mhs.txt", "w");
    fout_py   = fopen("hist/adj_mhs_hist_py.txt", "w");
    fout_R    = fopen("stat/adj_mhs_hist_r.txt", "w");
  }
